...
void loadEEPROMtable() {
  ...
  eepromReadArray((char *)&stored_CRC, EEPROM_program_table_CRC_addr,
  ...
  if (calculated_CRC != stored_CRC) {
    ...
    eepromReadArray((char *)&table, 0, sizeof(EEPROM_program_table_t));
...
void clearEEPROMtable() {
  ...
  for (int index = 0; index < 64; index++) {
    ...
    table.cell[index].type = 69;
  }

  eepromWriteArray((char *)&table, 0, sizeof(EEPROM_program_table_t));
  unsigned long calculated_CRC = eepromCRC(0, sizeof(EEPROM_program_table_t));
  eepromWriteArray((char *)&calculated_CRC, EEPROM_program_table_CRC_addr,
                   sizeof(unsigned long));
}
