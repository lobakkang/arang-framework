...
#define NVMCTRL_CMD_COPY_gc                                                    \
  (NVMCTRL_CMD_gm + 1) // one beyond existing commands.
...
void optiboot_writePage(const uint8_t allocated_flash_space[],
  ...
  optiboot_page_erase(
      (optiboot_addr_t)(void *)&allocated_flash_space[SPM_PAGESIZE *
                                                      (page - 1)]);
  ...
  for (uint16_t i = 0; i < SPM_PAGESIZE; i++) {
    if (i % 2 == 0) // We must write words
      ...
      optiboot_page_fill(
          (optiboot_addr_t)(void *)&allocated_flash_space[i + SPM_PAGESIZE *
                                                                  (page - 1)],
          word_buffer);
    }
  }
 ...
 *
 * Currently, there are no mega0/xTint parts with more than 64k, and when there
 * are they'll need extra effort beyond just RAMPZ :-(
 */
...
void optiboot_writePage(const uint8_t allocated_flash_space[],
  ...
  for (uint16_t i = 0; i < SPM_PAGESIZE; i++) {
    ...
    do_nvmctrl((optiboot_addr_t)(void *)adjusted_address, NVMCTRL_CMD_COPY_gc,
               data_to_store[i]);
  }
...
void optiboot_readPage(const uint8_t allocated_flash_space[],
  ...
  for (uint16_t j = 0; j < SPM_PAGESIZE; j++) {
    read_character =
        pgm_read_byte(&allocated_flash_space[j + SPM_PAGESIZE * (page - 1)]);
